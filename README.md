...
| `--restir / --no-restir` | 有効 | ReSTIR直接照明（低サンプルでも滑らか。無効時は従来NEE） |
...
1画素を8または32レーンで分担し、GPU向け32bit乱数生成とfloat演算を使います。
 拡散反射、GGX金属、粗い／滑らかな屈折、発光、直接照明・MIS、クランプ、適応サンプリング、
 ReSTIR（RIS＋画面空間でのreservoir再利用による直接照明の分散低減）を実装しています。
 既定で有効のため、低サンプル数でも滑らかに描画します（`--no-restir` で無効化）。
 動画では前フレームのreservoirを再投影して再利用するtemporal結合も行います。
 最終段にはG-buffer（位置・法線・アルベド）参照のエッジ保持型時空間デノイザがかかり、
 間接光・グロッシー・ガラスの残存ノイズも抑えます。屈折など視点依存の強い材質は
 時間ブレンドの対象外とし、空間フィルタのみ適用するため、カメラ移動時の尾引きは出ません。環境変数 `RAYMOTION_DENOISE=0` で
 デノイズのみ、`RAYMOTION_RESTIR_TEMPORAL=0` で時間方向の再利用のみを無効化できます。
...
走査用float/BVH4キャッシュも更新します。構造変更時と32回のジオメトリ更新ごとにSAHで再構築します。
後者は長時間の移動による木の品質劣化を抑えるためです。同じモデル・配置・材質のフレームでは、座標変換とCPU側BVH更新を省略します。性能計測は拡張機能のREADMEを参照してください。
...
再生／一時停止、右隣の停止ボタンを備え、停止後はビルドの更新を確認し、変更がなければ再利用します。
